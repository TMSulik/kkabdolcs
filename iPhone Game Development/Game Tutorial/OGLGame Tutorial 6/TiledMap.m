...
- (id)initWithTiledFile:(NSString*)tiledFile fileExtension:(NSString*)fileExtension {
	...
	if (self != nil) {
		...
		NSString *tiledXML = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:tiledFile ofType:fileExtension]];
...
- (void)parseMapFile:(NSString*)tiledXML {
	...
	for(DDXMLElement *layerElement in layerElements) {
		...
		NSLog(@"----> Found '%d' tile elements", [tileElements count]);
