...
//  iPhoneGame
//
//  Created by Seonghyeon Choe on 1/22/10.
...
@interface Image ()
- (void)initImpl;
- (void)renderAt:(CGPoint)point
	   texCoords:(GLfloat[])coordinates
	quadVertices:(GLfloat[])vertices;
@end
...
- (NSString *)description {
	return [NSString stringWithFormat:@"texture:%d width:%d height:%d texWidth:%d texHeight:%d maxTexWidth:%f maxTexHeight:%f angle:%f scale:%f", 
			[texture name], imageWidth, imageHeight, textureWidth, textureHeight, maxTexWidth, maxTexHeight, rotation, scale];
}


- (Image*)getSubImageAtPoint:(CGPoint)point 
			   subImageWidth:(GLuint)subImageWidth 
			  subImageHeight:(GLuint)subImageHeight 
					   scale:(float)subImageScale 
{
...
- (void)renderSubImageAtPoint:(CGPoint)point
					   offset:(CGPoint)offsetPoint
				subImageWidth:(GLfloat)subImageWidth
			   subImageHeight:(GLfloat)subImageHeight
				centerOfImage:(BOOL)center
{
...
- (void)renderAt:(CGPoint)point
	...
	quadVertices:(GLfloat[])quadVertices
{
