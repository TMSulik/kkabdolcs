...
/*
 ...
 07/05/09 - Kerning functionality added based on the kerning support in Cocos2D
 */
...
- (id)initWithFontImageNamed:(NSString*)fontImage controlFile:(NSString*)controlFile scale:(float)fontScale filter:(GLenum)filter {
	...
	if (self != nil) {
		...
		// Game state
		shareGameState = [SingletonGameState sharedGameStateInstance];
		
		// Reference the font image which has been supplied and which contains the character bitmaps
		image = [[Image alloc] initWithImage:[UIImage imageNamed:fontImage] scale:fontScale filter:filter];
		// Set the scale to be used for the font
		scale = fontScale;
		
		// Set the kerningDectionary to nil.  This will be used when processing any kerning info
...
- (void)parseFont:(NSString*)controlFile {
	...
	NSString *contents = [NSString stringWithContentsOfFile:[[NSBundle mainBundle] pathForResource:controlFile ofType:@"fnt"]];
...
- (void)drawStringAt:(CGPoint)point text:(NSString*)text {
	...
	if([[image texture] name] != [shareGameState currentlyBoundTexture]) {
		[shareGameState setCurrentlyBoundTexture:[[image texture] name]];
...
- (void)setColourFilterRed:(float)red green:(float)green blue:(float)blue alpha:(float)alpha {
	...
	[image setColourFilterRed:red green:green blue:blue alpha:alpha];
}
