...
//  iPhoneGame
//
//  Created by Seonghyeon Choe on 2/8/10.
...
- (void) loadSoundWithKey:(NSString*)theSoundKey 
	...
	NSUInteger *outData = malloc(fileSize);
	...
	if(result != 0) {
		NSLog(@"ERROR SoundManager: Cannot load sound: %@", theFileName);
		return; 
	}
...
- (AudioFileID) openAudioFile:(NSString*)theFilePath {
	...
	if(result != 0)	{
		NSLog(@"ERROR SoundManager: Cannot open file: %@", theFilePath);
		return nil;
	}
